...
static int dfs(Graph *graph, AdjList *adjlist, const void *exit, Set *visited) {
    /* Check if this vertex is the exit. */
    if (graph->match(adjlist->vertex, exit)) {
        return 1;
    }

    /* Add this vertex to the visited set. */
    set_insert(visited, adjlist->vertex);

    /* Recursively apply DFS to unvisited adjacent vertices. */
    ListElmt *element;
    void *adjacent;
    AdjList *adjlist_next;

    for (element = list_head(&adjlist->adjacent); element != NULL; element = list_next(element)) {
        adjacent = list_data(element);
        if (!set_is_member(visited, adjacent)) {
            if (graph_adjlist(graph, adjacent, &adjlist_next) == 0 && dfs(graph, adjlist_next, exit, visited)) {
                return 1;
            }
        }
    }
...
int isExitReachable(Graph *pMaze, char entrance, char exit) {
    Set visited;
    AdjList *adjlist;
    char entrance_vertex = entrance;
    char exit_vertex = exit;

    /* Initialize the visited set. */
    set_init(&visited, pMaze->match, NULL);

    /* Start DFS from the entrance vertex. */
    if (graph_adjlist(pMaze, &entrance_vertex, &adjlist) == 0) {
        int result = dfs(pMaze, adjlist, &exit_vertex, &visited);
        set_destroy(&visited);
        return result;
    }
